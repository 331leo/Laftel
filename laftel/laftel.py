...
async def _get_AnimeInfo_by_rawdata(data: dict):
    ...
    obj.air_time = _animation_info.get("original_air_time", None) if _animation_info.get("original_air_time", None) else _animation_info.get("distributed_air_time", None)
    ...
    if not obj.id: return None
    return obj

async def get_AnimeInfo(id: int):
    return await _get_AnimeInfo_by_rawdata(await api_request(ApiUrl.AnimeItem.format(id=id)))
...
async def search_Anime(query: str):
    ...
    return_list = []
    for data in datas:
        return_list.append(await get_AnimeInfo(data.get("id")))
    return return_list
...
class sync:
    ...
    def search_Anime(query: str, loop=asyncio.get_event_loop()):
        return loop.run_until_complete(search_Anime(query))